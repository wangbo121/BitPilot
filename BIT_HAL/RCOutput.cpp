...
#include "HAL.h"
  ...
  void AP_HAL::RCOutput::init()
  {

  }

    /* Output freq (1/period) control */
     void     AP_HAL::RCOutput:: set_freq(uint32_t chmask, uint16_t freq_hz)
     {

     }

     uint16_t  AP_HAL::RCOutput::get_freq(uint8_t ch)
     {
     ...
     * or a mask of channels */
     void     AP_HAL::RCOutput:: enable_ch(uint8_t ch)
     {

     }

     void     AP_HAL::RCOutput:: disable_ch(uint8_t ch)
     {
     ...
     */
     void     AP_HAL::RCOutput:: write(uint8_t ch, uint16_t period_us)
     {
     ...
     * reported */
     uint16_t  AP_HAL::RCOutput::read(uint8_t ch)
     {

     }
     void      AP_HAL::RCOutput::read(uint16_t* period_us, uint8_t len)
     {

     }

    /* Read the current input state. This returns the last value that was written. */
     uint16_t  AP_HAL::RCOutput::read_last_sent(uint8_t ch)
     {

     }
     void      AP_HAL::RCOutput::read_last_sent(uint16_t* period_us, uint8_t len)
     {
    ...
    /*
      ...
      in the safe state
     */
     void      AP_HAL::RCOutput::set_safety_pwm(uint32_t chmask, uint16_t period_us)
     {
     ...
     */
     void      AP_HAL::RCOutput::set_failsafe_pwm(uint32_t chmask, uint16_t period_us) {}
     ...
     */
     bool      AP_HAL::RCOutput::force_safety_on(void) { return false; }

    /*
      force the safety switch off, enabling PWM output from the IO board
     */
     void    AP_HAL::RCOutput::  force_safety_off(void) {}

    /*
      If we support async sends (px4), this will force it to be serviced immediately
     */
     void      AP_HAL::RCOutput::force_safety_no_wait(void) {}
     ...
     */
     void      AP_HAL::RCOutput::set_esc_scaling(uint16_t min_pwm, uint16_t max_pwm) {}

    /*
      returns the pwm value scaled to [-1;1] regrading to set_esc_scaling ranges range without constraints.
     */
     float    AP_HAL::RCOutput:: scale_esc_to_unity(uint16_t pwm) { return 0; }

    /*
      enable SBUS out at the given rate
     */
     bool      AP_HAL::RCOutput::enable_sbus_out(uint16_t rate_gz) { return false; }

    /*
     * Optional method to control the update of the motors. Derived classes
     * can implement it if their HAL layer requires.
     */
     void  AP_HAL::RCOutput::timer_tick(void) { }


     void     AP_HAL::RCOutput::set_output_mode(enum output_mode mode) {}
